...
def test_queue_length_recorder_tracks_length():
    ...
    env.register(q); env.register(rec)
    q.enqueue(sw.Entity()); q.enqueue(sw.Entity())
    env.run(until=2.0)
    q.dequeue()
    env.run(until=5.0)
...
def test_service_utilisation_recorder_shapes():
    ...
    env.register(gen); env.register(svc); env.register(sink); env.register(rec)
    env.run()
    n = len(rec.times)
    assert n > 1
...
def test_plot_service_utilisation_returns_figure(plotly_go):
    ...
    rec = sw.ServiceUtilisationRecorder(svc)
    env.register(svc); env.register(sink); env.register(rec)
